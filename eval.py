...
class Evaluator(object):
	...
	def wrap_venv(venv, env_name, device='cpu'):
		...
		is_cheetah = env_name.startswith('HalfCheetah')
	...
	def evaluate(self, 
		...
		for env_name in self.env_names:
			...
			if accumulator == 'mean':
				stats[f"mean_test_returns:{env_name}"] = np.mean(env_returns[env_name])
				stats[f"cvar5_test_returns:{env_name}"] = cvar(env_returns[env_name], 0.05)
			else:
...
def main(args=None):
	...
	if not args.base_path:
		args.base_path = args.log_dir
	if not args.env_names:
		args.env_names = args.test_env_names
	...
	if num_envs*args.num_processes > args.max_num_processes:
		if args.num_processes > args.max_num_processes:
			args.num_processes = args.max_num_processes
			chunk_size = 1
		else:
	...
	for k,_ in stats.items():
		...
		output_results[f'n_episodes'] = f'{len(results):d}'
		output_results[k] = f'{np.mean(results):.2f} +/- {np.std(results)/np.sqrt(len(results)):.2f}'
		output_results[f'cvar5_{k}'] = f'{cvar(results, 0.05):.2f}'
	...
	if args.use_wandb:
		res_path = os.path.join(wandb.run.dir, f'{k}.pkl')
	else:
		res_path = os.path.join(xpid_dir, f'{k}.pkl')
	with open(res_path, 'wb') as h:
		pkl.dump(output_results, h)
